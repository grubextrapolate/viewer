...
GLOBAL(void)
write_JPEG_file (char * filename, int quality, TEXTURE *tex)
{
...
TEXTURE *read_JPEG_file (char *filename) {
   ...
   debug("reading jpeg from file \"%s\"\n", filename);
   ...
   while (cinfo.output_scanline < cinfo.output_height) {
      ...
      for (i = 0; i < img_width; i++) {
         tmp = *(buffer[i*cinfo.output_components]);
         *(tex->tex + (RGBA*((img_height-1-row)*img_width + i))) =
                   (GLubyte) tmp;
         tmp = *(buffer[i*cinfo.output_components + 1]);
         *(tex->tex + (RGBA*((img_height-1-row)*img_width + i)+1)) =
                   (GLubyte) tmp;
         tmp = *(buffer[i*cinfo.output_components + 2]);
         *(tex->tex + (RGBA*((img_height-1-row)*img_width + i)+2)) =
                   (GLubyte) tmp;
      }
      row++;
   }
